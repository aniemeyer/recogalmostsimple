...
##                                                           Alice Niemeyer
##
##  Copyright 2005-2006 Lehrstuhl D fï¿½r Mathematik, RWTH Aachen
##
...
#sg
    ...
    if recognise.needForms <> true then return NotApplicable; fi;
...
RECOG.MeatAxe := function( recognise, grp )
    ...
    if recognise.needMeataxe <> true then return NotApplicable; fi;
...
#sg
    ...
    if recognise.IsSpContained = false then return false; fi;
...
#sg
    ...
    if recognise.IsSUContained = false then return false; fi;
...
#sg
    ...
    if recognise.IsSOContained = false then return false; fi;
...
# sg
            if r.isAbelian = true then
                Print("--------> abelian + not classical \n");
            fi;
	    if r.hasExp2 = true then
                Print("--------> exp(G/Z) = 2 + not classical \n");
            fi;
	    if r.isSubgroupOfGammaL = true then
                Print("--------> conjugate to a subgroup of gammaL(1,", r.q, "^2) + not classical \n");
            fi;
	    if r.isImprimitive = true then
                Print("--------> imprimitive + not classical \n");
            fi;
	    if r.isAlt5Alt4Sym4 = true then
                Print("--------> G/Z isomorphic to Alt5, Alt4 or Sym4 (and not one of the exeptions) + not classical \n");
            fi;
	    if r.isRepresentableOverSubfield = true then
                Print("--------> G is representable over a proper subfield + not classical \n");
            fi;
# sg
